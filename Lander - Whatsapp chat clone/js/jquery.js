...
function writing(e) {
    document.getElementById("chatbox").innerHTML =  document.getElementById("chatbox").innerHTML + '<tr class="lo" style="color:#aaa;"><td class="kate-msg"><div class="chat-bubble-l"><i> '+nametyping+' </i></div></td></tr>';
    setTimeout('window["' + e + '"]()', randomFromTo(2e3, 5e3))
}

function message(e) {
    var t = getDateTime();
    $(".lo").hide();
    document.getElementById("chatbox").innerHTML = document.getElementById("chatbox").innerHTML + '<tr><td class="kate-msg"><div class="chat-bubble-l"><div>' + e + '</div><div class="time">' + t + "</div></div></td></tr>";
	document.getElementById('popsound').play();
    document.getElementById("usermessage").focus();
}

function comeon(e, t) {
    var n = comeons[Math.floor(Math.random() * comeons.length)];
    message(n);
    waitformessage("+desired+", '+"next"+')
}

function waitformessage(e, t) {
    if (document.getElementById("messagecount").innerHTML.length >= e) {
        y = 1;
        z = 1;
        setTimeout("writing('" + t + "')", randomFromTo(2e3, 5e3))
    } else {
        y = y + 1;
        if (y >= 300) {
            y = 1;
        ...
        setTimeout("waitformessage(" + e + ', "' + t + '")', 100)
    }
}

function randomFromTo(e, t) {
    return Math.floor(Math.random() * (t - e + 1) + e)
}

function go() {
    setTimeout("writing('f1')", randomFromTo(1e3, 2e3));
    f13()
}
...
function f1() {
    ...
    setTimeout("waitformessage(1, 'f2')", 1e3)
}
...
function f2() {
    message( message1);
    document.getElementById("messagecount").innerHTML = "";
    setTimeout("waitformessage(1, 'f3')", 1e3)
}

function f3() {
    message(message2);
    setTimeout("writing('f4')", 5e3)
}
...
function f4() {
    ...
    message(message3 + e + "?");
    document.getElementById("messagecount").innerHTML = "";
    setTimeout("waitformessage(1, 'f5')", 1e3)
}

function f5() {
    message(message4);
    setTimeout("writing('f6')", 6e3)
}

function f6() {
    message(message5);
    setTimeout("waitformessage(1, 'f7')", 1e3)
}

function f7() {
    message(message6);
    setTimeout("writing('f8')", 5e3)
}

function f8() {
    message(message7);
    setTimeout("writing('f9')", 5e3)
}

function f9() {
    message('<a href="javascript:void()" onclick="msg()">'+contact_name+'  </a>'); //href=offer
    setTimeout("writing('f10')", 2e3)
}

function msg(){
	alert("Tu subscripción a Whatsapp ha finalizado!. Click Ok para renovarla");
	location.href=offer;
}

function f10() {
    message(message8);
    setTimeout("writing('f11')", 5e3)
}
...
function f12() {
    ...
    if (t == 10 && document.domain == "google.com") {
        window.location.href = "http:\\www.google.com"
    }
}

function away() {
    document.getElementById("chatbox").innerHTML = document.getElementById("chatbox").innerHTML + '<tr style="color:#aaa;"><td colspan="2"><i>'+guest_chat_timeout+' <a href="' + offer + '">'+setup_profile+'</a> '+to_chat_more+' </i></td></tr>';
    $("#timeout").val(1)
}
...
function f13() {
    ...
    if (n < 19 && document.domain == "google.com") {
        window.location.href = "http:\\www.google.com"
    }
}
