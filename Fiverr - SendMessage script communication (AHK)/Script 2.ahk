...
Gui, Show, w571 h324,Script 2 ;we show the user interface then
...
TargetScriptTitle = Script 1 ;we name the script window. NB: IF THE SCRIPT HAS NO WINDOW USE "Receiver.ahk ahk_class AutoHotkey"
if SEND_STRING(StringToSend, TargetScriptTitle) ;We send the string
	MsgBox Completed sending string to script 1 ;we prompt of on success
else 
	MsgBox We failed to send the string to script 1 ;we prompt if we failed
return
...
test_function_2:
StringToSend := "CallFunction,function_in_script2_add,2,5" ;should add 2 + 5
TargetScriptTitle = Script 1 ;we name the script window. NB: IF THE SCRIPT HAS NO WINDOW USE "Receiver.ahk ahk_class AutoHotkey"
if SEND_STRING(StringToSend, TargetScriptTitle) ;We send the string
	MsgBox Completed sending string to script 1 ;we prompt of on success
else 
...
;these functions we need to cal using script 1
function_in_script2(){
	MsgBox,0,Script 2,This has been generated by a function in script 2
}

function_in_script2_add(a,b){
	c := a + b
	MsgBox,0,Script 2,This has been generated by a function in script 2`n`nThe answer for %a% + %b% = %c%
}
